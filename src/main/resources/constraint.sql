...
alter table task add constraint backlog_ref foreign key (backlog_id) references back_log(id) on delete cascade on update cascade ,
                 add constraint sprint_ref foreign key (sprint_id) references sprint (id) on delete set null on update cascade ,
                 add constraint extra_info_ref foreign key (extra_info_id)  references extra_task_info (id) on delete set null,
                 add constraint creator_ref foreign key (creator_id) references usr (id) on delete set null on update cascade;
